...
"""Small WebRTC connection implementation for Blink.
...
from blink.utils.base_object import BaseObject
...
class SmallWebRTCConnection(BaseObject):
    ...
    async def connect(self):
        ...
        if self.is_connected():
            ...
            if video_input_track or screen_video_input_track:
                logger.debug(
                    "Skipping automatic WebRTC renegotiation for an already-connected peer; "
                    "reload or reconnect the browser page if media stalls."
                )
    ...
    def replace_audio_track(self, track) -> bool:
        ...
        Returns:
            True when an audio sender accepted the track, otherwise False.
        """
    ...
    def replace_video_track(self, track) -> bool:
        ...
        Returns:
            True when a video sender accepted the track, otherwise False.
        """
    ...
    def is_media_transport_ready(self) -> bool:
        """Return whether media sender tracks can still be used.

        App messages use the data-channel heartbeat because they need a live data
        channel. Audio/video RTP sender tracks should not be dropped solely
        because browser pings are delayed; a busy local model or store query can
        delay those pings while the peer connection is still able to carry media.
        """
        if not self._connect_invoked:
            return False
        return self._pc.connectionState == "connected"
    ...
    def send_app_message(self, message: Any):
        ...
        * The queue will not grow beyond ``MAX_MESSAGE_QUEUE_SIZE`` entries.
        ...
        if self._data_channel_ready():
            try:
                self._data_channel.send(json_message)
            except ConnectionError as exc:
                logger.debug(f"Data channel send failed; disabling queueing: {exc}")
                self._data_channel_enabled = False
                self._outgoing_messages_queue.clear()
        elif self._data_channel and self._data_channel.readyState == "open":
            logger.debug("Data channel is open but peer connection is unavailable; dropping message.")
            self._data_channel_enabled = False
            self._outgoing_messages_queue.clear()
    ...
    def _flush_message_queue(self):
        ...
        while self._outgoing_messages_queue:
            if not self._data_channel_ready():
                logger.debug("Data channel became unavailable while flushing queued messages.")
                self._data_channel_enabled = False
                self._outgoing_messages_queue.clear()
                break
            ...
            try:
                ...
                logger.debug(f"Data channel flush failed; dropping queued messages: {exc}")
                self._data_channel_enabled = False
                self._outgoing_messages_queue.clear()
                break

    def _data_channel_ready(self) -> bool:
        return bool(
            self._data_channel
