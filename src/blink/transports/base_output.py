...
"""Base output transport implementation for Blink.
...
from blink.audio.dtmf.utils import load_dtmf_audio
from blink.audio.mixers.base_audio_mixer import BaseAudioMixer
from blink.audio.utils import create_stream_resampler, is_silence
...
from blink.utils.frame_queue import FrameQueue
from blink.utils.time import nanoseconds_to_seconds
...
class BaseOutputTransport(FrameProcessor):
    ...
    async def interrupt_audio_output(self):
        """Abort any active audio write after an interruption.

        Transports with blocking playback APIs can override this to stop the
        current write immediately. The default is intentionally a no-op so
        existing transports keep their current behavior.
        """
        pass
    ...
    class MediaSender:
        ...
        async def handle_interruptions(self, _: InterruptionFrame):
            ...
            """
            await self._transport.interrupt_audio_output()
