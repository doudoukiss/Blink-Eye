...
"""Tavus transport implementation for Blink.
...
from blink.transports.daily.transport import (
    DailyCallbacks,
    DailyParams,
...
class TavusTransportClient:
    """Transport client that integrates Blink with the Tavus platform.

    A transport client that integrates a Blink Bot with the Tavus platform by managing
    ...
    is started via `TavusApi`, Tavus provides a `roomURL` that can be used to connect the Blink Bot
    ...
    def __init__(
        ...
        *,
        ...
        api_key: str,
        replica_id: str,
        persona_id: str = "blink-stream",
        session: aiohttp.ClientSession,
    ) -> None:
        """Initialize the Tavus transport client.

        Args:
            bot_name: The name of the Blink bot instance.
            ...
            persona_id: ID of the Tavus persona. Defaults to "blink-stream",
                which signals Tavus to use the TTS voice of the Blink bot
    ...
    async def setup(self, setup: FrameProcessorSetup):
        ...
        try:
            ...
            self._client = DailyTransportClient(
                room_url, None, "Blink", self._params, daily_callbacks, self._bot_name
            )
...
class TavusTransport(BaseTransport):
    ...
    When used, the Blink bot joins the same virtual room as the Tavus Avatar and the user.
