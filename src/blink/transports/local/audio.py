...
"""Local audio transport implementation for Blink.
...
from blink.frames.frames import InputAudioRawFrame, OutputAudioRawFrame, StartFrame
...
try:
    ...
    logger.error(
        "In order to use local audio, you need to `pip install blink-ai[local]`. On MacOS, you also need to `brew install portaudio`."
    )
...
class LocalAudioOutputTransport(BaseOutputTransport):
    ...
    def _ensure_executors(self):
        if getattr(self._executor, "_shutdown", False):
            self._executor = ThreadPoolExecutor(max_workers=1)
        if getattr(self._control_executor, "_shutdown", False):
            self._control_executor = ThreadPoolExecutor(max_workers=1)
    ...
    async def write_audio_frame(self, frame: OutputAudioRawFrame) -> bool:
        ...
        if self._out_stream:
            try:
                ...
                logger.debug(f"Local audio output write stopped or failed: {e}")
        return False

    async def interrupt_audio_output(self):
        """Stop active PyAudio playback immediately after a user interruption."""
        if not self._out_stream:
            return

        await self.get_event_loop().run_in_executor(
            self._control_executor, self._interrupt_output_stream
        )

    def _interrupt_output_stream(self):
        stream = self._out_stream
        if not stream:
            return

        try:
            # PyAudio does not expose Stream.abort_stream(), but its C extension
            # does. Use it first so a blocking write returns quickly; fall back
            # to the public stop_stream() path on runtimes that hide internals.
            raw_stream = getattr(stream, "_stream", None)
            abort_stream = getattr(getattr(pyaudio, "pa", None), "abort_stream", None)
            if raw_stream is not None and abort_stream:
                abort_stream(raw_stream)
                if hasattr(stream, "_is_running"):
                    stream._is_running = False
            elif stream.is_active():
                stream.stop_stream()
        except Exception as e:
            logger.debug(f"Unable to interrupt local audio output stream: {e}")

        try:
            if not stream.is_active():
                stream.start_stream()
        except Exception as e:
            logger.debug(f"Unable to restart local audio output stream after interruption: {e}")
