...
from blink.transports.whatsapp.api import (
    WhatsAppApi,
    WhatsAppConnectCall,
...
class WhatsAppClient:
    ...
    async def terminate_all_calls(self) -> None:
        ...
        termination_tasks = []
        for call_id, webrtc_connection in self._ongoing_calls_map.items():
            ...
            termination_tasks.append(webrtc_connection.disconnect())
    ...
    async def _handle_connect_event(self, call: WhatsAppConnectCall) -> SmallWebRTCConnection:
        ...
        try:
            ...
            webrtc_connection = SmallWebRTCConnection(self._ice_servers)
            await webrtc_connection.initialize(sdp=call.session.sdp, type=call.session.sdp_type)
            sdp_answer = webrtc_connection.get_answer().get("sdp")
            ...
            self._ongoing_calls_map[call.id] = webrtc_connection
    ...
    async def _handle_terminate_event(self, call: WhatsAppTerminateCall) -> bool:
        ...
        try:
            if call.id in self._ongoing_calls_map:
                webrtc_connection = self._ongoing_calls_map[call.id]
