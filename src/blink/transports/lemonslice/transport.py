...
"""LemonSlice transport for Blink.
...
from blink.transports.lemonslice.api import LemonSliceApi
...
class LemonSliceTransportClient:
    """Transport client that integrates Blink with the LemonSlice platform.

    A transport client that integrates a Blink Bot with the LemonSlice platform by managing
    ...
    async def setup(self, setup: FrameProcessorSetup):
        ...
        try:
            ...
            self._daily_transport_client = DailyTransportClient(
                room_url, None, self._bot_name, self._params, daily_callbacks, "LemonSliceBlink"
            )
...
class LemonSliceTransport(BaseTransport):
    ...
    When used, the Blink bot joins the same virtual room as the LemonSlice Avatar and the user.
