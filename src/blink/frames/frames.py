...
"""Core frame definitions for the Blink framework.

This module contains all frame types used throughout the Blink pipeline system,
...
from blink.audio.dtmf.types import KeypadEntry
from blink.audio.turn.base_turn_analyzer import BaseTurnParams
from blink.audio.vad.params import VADParams
from blink.metrics.metrics import MetricsData
from blink.transcriptions.language import Language
from blink.utils.text.base_text_aggregator import AggregationType
from blink.utils.time import nanoseconds_to_str
from blink.utils.utils import obj_count, obj_id

if TYPE_CHECKING:
    from blink.processors.aggregators.llm_context import LLMContext, LLMContextMessage, NotGiven
    from blink.processors.frame_processor import FrameProcessor
    from blink.services.settings import ServiceSettings
    from blink.utils.context.llm_context_summarization import LLMContextSummaryConfig
...
@dataclass
class Frame:
    """Base frame class for all frames in the Blink pipeline.
...
class LLMSummarizeContextFrame(ControlFrame):
    ...
    Parameters:
        config: Optional per-request override for summary generation settings
            ...
            summarizer's default :class:`~blink.utils.context.llm_context_summarization.LLMContextSummaryConfig`
            is used.
    """
...
class ServiceUpdateSettingsFrame(ControlFrame, UninterruptibleFrame):
    ...
    Parameters:
        ...
        delta: :class:`~blink.services.settings.ServiceSettings` delta-mode
