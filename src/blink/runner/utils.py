...
Example::
    ...
    from blink.runner.utils import parse_telephony_websocket
...
def _transport_identity(transport: BaseTransport) -> tuple[str, str]:
    """Return the transport class module and name without importing optional extras."""
    transport_type = type(transport)
    return transport_type.__module__, transport_type.__name__


def _transport_matches(
    transport: BaseTransport, *, class_name: str, module_names: tuple[str, ...]
) -> bool:
    """Return True when a transport matches a supported module/class identity."""
    module_name, actual_class_name = _transport_identity(transport)
    return actual_class_name == class_name and module_name in module_names
...
def get_transport_client_id(transport: BaseTransport, client: Any) -> str:
    ...
    """
    if _transport_matches(
        transport,
        class_name="SmallWebRTCTransport",
        module_names=("blink.transports.smallwebrtc.transport",),
    ):
        return client.pc_id
    ...
    if _transport_matches(
        ...
        module_names=("blink.transports.daily.transport",),
    ):
        return client["id"]
...
async def maybe_capture_participant_camera(
    ...
    if _transport_matches(
        ...
        await transport.capture_participant_video(video_source="camera", framerate=framerate)
...
async def maybe_capture_participant_screen(
    ...
    if _transport_matches(
        ...
        await transport.capture_participant_video(video_source="screenVideo", framerate=framerate)
...
async def _create_telephony_transport(
    ...
    """
    from blink.transports.websocket.fastapi import FastAPIWebsocketTransport
    ...
    if transport_type == "twilio":
        from blink.serializers.twilio import TwilioFrameSerializer
        ...
        from blink.serializers.telnyx import TelnyxFrameSerializer
        ...
        from blink.serializers.plivo import PlivoFrameSerializer
        ...
        from blink.serializers.exotel import ExotelFrameSerializer
...
async def create_transport(
    ...
    if isinstance(runner_args, DailyRunnerArguments):
        ...
        from blink.transports.daily.transport import DailyTransport
        ...
        from blink.transports.livekit.transport import LiveKitTransport
