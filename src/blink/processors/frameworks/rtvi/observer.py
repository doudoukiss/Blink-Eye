...
import blink.processors.frameworks.rtvi.models as RTVI
...
from blink.metrics.metrics import (
    LLMUsageMetricsData,
...
def _calculate_audio_volume(audio: bytes, sample_rate: int) -> float:
    """Lazily import loudness helpers to keep core RTVI imports provider-light."""
    from blink.audio.utils import calculate_audio_volume

    return calculate_audio_volume(audio, sample_rate)
...
class RTVIObserver(BaseObserver):
    ...
    async def on_push_frame(self, data: FramePushed):
        ...
        if (
            ...
            if diff_time > self._params.audio_level_period_secs:
                level = _calculate_audio_volume(frame.audio, frame.sample_rate)
    ...
    async def _handle_aggregated_llm_text(self, frame: AggregatedTextFrame):
        ...
        if getattr(frame, "blink_subtitle_immediate", False):
            await self._send_aggregated_llm_text(frame)
        elif self._bot_is_speaking:
    ...
    async def _send_aggregated_llm_text(self, frame: AggregatedTextFrame):
        ...
        text = str(getattr(frame, "blink_display_text", None) or frame.text)
