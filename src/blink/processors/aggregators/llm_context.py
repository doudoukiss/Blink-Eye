...
"""Universal LLM context management for LLM services in Blink.

Context contents are represented in a universal format (based on OpenAI)
that supports a union of known Blink LLM service functionality.
...
try:
    ...
    class OpenAINotGiven:
        """Fallback OpenAI-style sentinel type when the SDK is unavailable."""

    OPEN_AI_NOT_GIVEN = OpenAINotGiven()
    ChatCompletionMessageParam = dict[str, Any]
    ChatCompletionToolChoiceOptionParam = dict[str, Any] | str

from blink.adapters.schemas.tools_schema import ToolsSchema
from blink.frames.frames import AudioRawFrame
