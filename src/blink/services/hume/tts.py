...
try:
    ...
    logger.error("In order to use Hume, you need to `pip install blink-ai[hume]`.")
...
DEFAULT_HEADERS = {
    "X-Hume-Client-Name": "blink",
    "X-Hume-Client-Version": blink_version(),
}
...
class HumeTTSService(TTSService):
    ...
    using the Python SDK and emits ``TTSAudioRawFrame`` frames suitable for Blink transports.
