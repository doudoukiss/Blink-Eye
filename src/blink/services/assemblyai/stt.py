...
from blink.services.settings import NOT_GIVEN, STTSettings, _NotGiven
from blink.services.stt_latency import ASSEMBLYAI_TTFS_P99
from blink.services.stt_service import WebsocketSTTService
from blink.transcriptions.language import Language
from blink.utils.time import time_now_iso8601
from blink.utils.tracing.service_decorators import traced_stt
...
try:
    ...
    logger.error('In order to use AssemblyAI, you need to `pip install "blink-ai[assemblyai]"`.')
    raise Exception(f"Missing module: {e}")


def map_language_from_assemblyai(language_code: str) -> Language:
    """Map AssemblyAI language codes to Blink Language enum.
...
class AssemblyAISTTService(WebsocketSTTService):
        ...
        **kwargs,
        ...
        Args:
            ...
            vad_force_turn_endpoint: Controls turn detection mode.
                When True (Blink mode, default): Forces AssemblyAI to return finals ASAP
                so Blink's turn detection (e.g., Smart Turn) decides when the user is done.
            ...
            ttfs_p99_latency: P99 latency from speech end to final transcript in seconds.
                Override for your deployment. See https://github.com/blink-ai/stt-benchmark
        ...
        # 6. Configure Blink turn mode (mutates default_settings)
        if vad_force_turn_endpoint:
            self._configure_blink_turn_mode(default_settings, is_u3_pro)
    ...
    def _configure_blink_turn_mode(self, settings: Settings, is_u3_pro: bool):
        """Configure settings for Blink turn detection mode.

        When vad_force_turn_endpoint is enabled, force AssemblyAI to return
        finals as fast as possible so Blink's smart turn analyzer can decide
        when the user is done speaking. VAD stop is the absolute ceiling.

        u3-rt-pro:
        - min_turn_silence defaults to 100ms (user can override)
        - max_turn_silence is ALWAYS set equal to min_turn_silence
          to avoid double turn detection (AssemblyAI + Blink both analyzing)
        ...
        if is_u3_pro:
            ...
            if settings.max_turn_silence is not None:
                logger.warning(
                    ...
                    f"OVERRIDDEN in Blink mode (vad_force_turn_endpoint=True). It will be set to "
    ...
    async def _connect_websocket(self):
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": f"AssemblyAI/1.0 (integration=Blink/{blink_version()})",
            }
    ...
    async def _handle_speech_started(self, message: SpeechStartedMessage):
        ...
        Blink turn detection, VAD + smart turn analyzer handle interruptions.
        """
        if self._vad_force_turn_endpoint:
            return  # Blink mode: handled by aggregator
    ...
    async def _handle_transcription(self, message: TurnMessage):
        ...
        Blink turn detection (vad_force_turn_endpoint=True):
