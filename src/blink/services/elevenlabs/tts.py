...
try:
    ...
    logger.error("In order to use ElevenLabs, you need to `pip install blink-ai[elevenlabs]`.")
...
class ElevenLabsTTSService(WebsocketTTSService):
    ...
    async def _close_context(self, context_id: str):
        # ElevenLabs requires that Blink explicitly closes contexts to free
        ...
        if context_id and self._websocket:
            ...
            try:
                # ElevenLabs requires that Blink manages the contexts and closes them
    ...
    async def on_audio_context_completed(self, context_id: str):
        ...
        exhausted, so Blink must explicitly close it with
...
class ElevenLabsHttpTTSService(TTSService):
    ...
    def language_to_service_language(self, language: Language) -> Optional[str]:
        """Convert Blink Language to ElevenLabs language code.
