...
try:
    ...
    logger.error("In order to use Simli, you need to `pip install blink-ai[simli]`.")
...
class SimliVideoService(AIService):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._blink_resampler: AudioResampler = None
        self._blink_resampler_event = asyncio.Event()
    ...
    async def _consume_and_process_audio(self):
        ...
        await self._blink_resampler_event.wait()
        audio_iterator = self._simli_client.getAudioStreamIterator()
        async for audio_frame in audio_iterator:
            resampled_frames = self._blink_resampler.resample(audio_frame)
                ...
                if audio_array.any():
                    await self.push_frame(
                        TTSAudioRawFrame(
                            ...
                            sample_rate=self._blink_resampler.rate,
                            num_channels=1,
                        ),
                    )
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, TTSAudioRawFrame):
            ...
            try:
                ...
                if self._blink_resampler is None:
                    self._blink_resampler = AudioResampler(
                        "s16", old_frame.layout, old_frame.sample_rate
                    )
                    self._blink_resampler_event.set()
