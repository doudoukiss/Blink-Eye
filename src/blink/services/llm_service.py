...
from blink.adapters.base_llm_adapter import BaseLLMAdapter
from blink.adapters.schemas.direct_function import DirectFunction, DirectFunctionWrapper
...
from blink.function_calling import (
    FunctionCallHandler,
    FunctionCallParams,
    FunctionCallResultCallback,
)
from blink.processors.aggregators.llm_context import (
    LLMContext,
    LLMSpecificMessage,
)
from blink.processors.frame_processor import FrameDirection
from blink.services.ai_service import AIService
from blink.services.settings import LLMSettings
from blink.services.websocket_service import WebsocketService
from blink.turns.user_turn_completion_mixin import UserTurnCompletionLLMServiceMixin
from blink.utils.async_tool_cancellation import (
    ASYNC_TOOL_CANCELLATION_INSTRUCTIONS,
    CANCEL_ASYNC_TOOL_NAME,
    CANCEL_ASYNC_TOOL_SCHEMA,
)
from blink.utils.context.llm_context_summarization import (
    DEFAULT_SUMMARIZATION_TIMEOUT,
    LLMContextSummarizationUtil,
)

try:
    from blink.adapters.services.open_ai_adapter import OpenAILLMAdapter
except ModuleNotFoundError:  # pragma: no cover - optional provider seam
    class OpenAILLMAdapter(BaseLLMAdapter[dict[str, Any]]):
        """Fallback adapter placeholder when the OpenAI adapter is unavailable."""

        @property
        def id_for_llm_specific_messages(self) -> str:
            """Return the standard provider identifier for OpenAI-style messages."""
            return "openai"

        def get_llm_invocation_params(self, context: LLMContext, **kwargs) -> dict[str, Any]:
            """Raise when code attempts to use the unavailable OpenAI adapter."""
            raise ModuleNotFoundError("OpenAI adapter is unavailable in this environment")

        def to_provider_tools_format(self, tools_schema) -> List[Any]:
            """Raise when code attempts to use the unavailable OpenAI adapter."""
            raise ModuleNotFoundError("OpenAI adapter is unavailable in this environment")
