...
from blink.frames.frames import ErrorFrame, Frame, TranscriptionFrame
from blink.services.settings import NOT_GIVEN, STTSettings, _NotGiven
from blink.services.stt_service import SegmentedSTTService
...
if TYPE_CHECKING:
    try:
        ...
        logger.error("In order to use Whisper, you need to `pip install blink-ai[whisper]`.")
    ...
    try:
        ...
        logger.error(f"Exception: {e}")
        logger.error("In order to use Whisper, you need to `pip install blink-ai[mlx-whisper]`.")
        raise Exception(f"Missing module: {e}")
...
def language_to_whisper_language(language: Language) -> Optional[str]:
    """Maps Blink Language enum to Whisper language codes.
...
class WhisperSTTService(SegmentedSTTService):
    ...
    def language_to_service_language(self, language: Language) -> Optional[str]:
        """Convert from blink Language to Whisper language code.
