...
from blink.services.settings import NOT_GIVEN, TTSSettings, _NotGiven
from blink.services.tts_service import (
...
from blink.utils.text.skip_tags_aggregator import SkipTagsAggregator
...
try:
    ...
    logger.error("In order to use Rime, you need to `pip install blink-ai[rime]`.")
    raise Exception(f"Missing module: {e}")


def language_to_rime_language(language: Language) -> str:
    """Convert Blink Language to Rime language code.

    Args:
        language: The Blink Language enum value.
...
class RimeNonJsonTTSService(InterruptibleTTSService):
    """Blink TTS service for Rime's non-JSON WebSocket API.
    ...
    def language_to_service_language(self, language: Language) -> str:
        """Convert Blink Language enum to Rime language code.
