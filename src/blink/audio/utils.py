...
"""Audio utility functions for Blink.
...
various audio formats used in Blink pipelines.
"""
...
except ModuleNotFoundError:  # pragma: no cover - optional dependency seam
    pyln = None

from blink.audio.resamplers.base_audio_resampler import BaseAudioResampler
from blink.audio.resamplers.soxr_resampler import SOXRAudioResampler
...
def calculate_audio_volume(audio: bytes, sample_rate: int) -> float:
    ...
    if pyln is None:
        ...
        return normalize_value(float(rms), 0.0, 32768.0)
