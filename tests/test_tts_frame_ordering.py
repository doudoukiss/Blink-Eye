...
class MockBrowserChunkedHttpTTSService(TTSService):
    """Simulates browser pre-chunked HTTP TTS with one audio context per chunk."""
...
@pytest.mark.asyncio
async def test_browser_chunked_http_tts_uses_distinct_stop_boundaries_per_chunk():
    """Browser speech-director chunks need per-chunk TTS queue boundaries."""
    tts = MockBrowserChunkedHttpTTSService()
    ...
    down = frames_received[0]
    starts = [frame for frame in down if isinstance(frame, TTSStartedFrame)]
    audio = [frame for frame in down if isinstance(frame, TTSAudioRawFrame)]
    stops = [frame for frame in down if isinstance(frame, TTSStoppedFrame)]
    context_ids = [frame.context_id for frame in stops]

    assert len(starts) == 3
    assert len(audio) == 3
    assert len(stops) == 3
    assert len(set(context_ids)) == 3
    assert [frame.context_id for frame in starts] == context_ids
    assert [frame.context_id for frame in audio] == context_ids
