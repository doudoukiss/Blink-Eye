...
from blink.utils.string import match_endofsentence, parse_start_end_tags
...
class TestUtilsString(unittest.IsolatedAsyncioTestCase):
    async def test_endofsentence(self):
        ...
        assert match_endofsentence("My emails are foo@blink.dev and bar@blink.dev.") == 46
        assert match_endofsentence("My email is foo.bar@blink.dev.") == 30
        assert match_endofsentence("My email is spell(foo.bar@blink.dev).") == 37
        assert match_endofsentence("My email is <spell>foo.bar@blink.dev</spell>.") == 45
        ...
        assert not match_endofsentence("My emails are foo@blink.dev and bar@blink.dev")
...
class TestStartEndTags(unittest.IsolatedAsyncioTestCase):
    async def test_empty(self):
        ...
        assert parse_start_end_tags("Hello from Blink!", [], None, 0) == (None, 0)

    async def test_simple(self):
        # (<a>, </a>)
        assert parse_start_end_tags("Hello from <a>Blink</a>!", [("<a>", "</a>")], None, 0) == (
            None,
            24,
        )
        assert parse_start_end_tags("Hello from <a>Blink", [("<a>", "</a>")], None, 0) == (
            ("<a>", "</a>"),
            19,
        )
        assert parse_start_end_tags("Hello from <a>Blink", [("<a>", "</a>")], None, 6) == (
            ("<a>", "</a>"),
            19,
        )

        # (spell(, ))
        assert parse_start_end_tags("Hello from spell(Blink)!", [("spell(", ")")], None, 0) == (
            None,
            24,
        )
        assert parse_start_end_tags("Hello from spell(Blink", [("spell(", ")")], None, 0) == (
    ...
    async def test_multiple(self):
        ...
        assert parse_start_end_tags(
            "Hello from <a>Blink</a>! Hello <a>World</a>!", [("<a>", "</a>")], None, 0
        ) == (
            None,
            44,
        )
        ...
        assert parse_start_end_tags(
            "Hello from <a>Blink</a>! Hello <a>World", [("<a>", "</a>")], None, 0
        ) == (
            ("<a>", "</a>"),
            39,
        )
