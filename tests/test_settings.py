...
"""Tests for the typed settings infrastructure in blink.services.settings."""

from unittest.mock import patch

import pytest

from tests._optional import OPTIONAL_PROVIDER

try:
    from blink.services.deepgram.sagemaker.stt import DeepgramSageMakerSTTSettings
    from blink.services.deepgram.stt import DeepgramSTTService, DeepgramSTTSettings

    DEEPGRAM_SETTINGS_AVAILABLE = True
except ImportError:
    DeepgramSageMakerSTTSettings = DeepgramSTTService = DeepgramSTTSettings = None  # type: ignore[assignment]
    DEEPGRAM_SETTINGS_AVAILABLE = False

from blink.services.inworld.realtime import events as inworld_events
from blink.services.inworld.realtime.llm import InworldRealtimeLLMSettings
from blink.services.openai.realtime import events
from blink.services.openai.realtime.llm import OpenAIRealtimeLLMSettings
from blink.services.settings import (
    NOT_GIVEN,
    LLMSettings,
    ServiceSettings,
    STTSettings,
...
from blink.services.xai.realtime import events as grok_events
from blink.services.xai.realtime.llm import GrokRealtimeLLMSettings
...
@OPTIONAL_PROVIDER
@pytest.mark.skipif(not DEEPGRAM_SETTINGS_AVAILABLE, reason="deepgram extras not installed")
...
class TestDeepgramSTTSettingsExtraSync:
    ...
    def _make_service(self, **kwargs):
        with patch("blink.services.deepgram.stt.AsyncDeepgramClient"):
    ...
    def test_extra_synced_to_declared_field_at_init(self):
        ...
        from blink.services.deepgram.stt import LiveOptions
