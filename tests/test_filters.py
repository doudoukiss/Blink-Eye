...
from blink.processors.filters.function_filter import FunctionFilter
from blink.processors.filters.identity_filter import IdentityFilter
from blink.processors.filters.wake_check_filter import WakeCheckFilter
...
class TestFrameFilter(unittest.IsolatedAsyncioTestCase):
    async def test_text_frame(self):
        ...
        frames_to_send = [TextFrame(text="Hello Blink!")]
...
class TestWakeCheckFilter(unittest.IsolatedAsyncioTestCase):
    async def test_no_wake_word(self):
        filter = WakeCheckFilter(wake_phrases=["Hey, Blink"])
    ...
    async def test_wake_word(self):
        filter = WakeCheckFilter(wake_phrases=["Hey, Blink"])
        frames_to_send = [
            TranscriptionFrame(user_id="test", text="Hey, Blink", timestamp=""),
