...
from blink.pipeline.base_task import PipelineTaskParams
from blink.pipeline.parallel_pipeline import ParallelPipeline
from blink.pipeline.pipeline import Pipeline
from blink.pipeline.task import PipelineParams, PipelineTask
from blink.processors.filters.frame_filter import FrameFilter
from blink.processors.filters.identity_filter import IdentityFilter
from blink.processors.frame_processor import FrameDirection, FrameProcessor
from blink.tests.utils import HeartbeatsObserver, run_test
...
class TestPipeline(unittest.IsolatedAsyncioTestCase):
    async def test_pipeline_single(self):
        ...
        frames_to_send = [TextFrame(text="Hello from Blink!")]
...
class TestPipelineTask(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_idle_task_frames(self):
        ...
        async def delayed_frames():
            ...
            await task.queue_frame(TextFrame("Hello Blink!"))
            await asyncio.sleep(sleep_time_secs)
            await task.queue_frame(TextFrame("Hello Blink!"))
            await asyncio.sleep(sleep_time_secs)
            await task.queue_frame(TextFrame("Hello Blink!"))
    ...
    async def test_task_error(self):
        ...
        await task.queue_frame(TextFrame(text="Hello from Blink!"))
